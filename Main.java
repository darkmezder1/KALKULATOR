...
class Main {
    ...
    static String calc(String input) {
        ...
        if (parts.length != 3) {
            throw new IllegalArgumentException("нормально вводи цифры) а не буквы или что ты там вводишь");
        }
        ...
        switch (parts[1]) {
            ...
            case "/":
                if (num2 == 0) {
                    throw new IllegalArgumentException("На ноль делить нельзя дуралей ");
                }
                result = num1 / num2;
                break;
            default:
    ...
    static String convertBack(int number) {
        if (number < 1) {
            throw new IllegalArgumentException("в римских цифрах нет отрицательных ))");
        }
